...
public final class Prefix implements
  ...
  public static Prefix valueOf(Path base, int wildcards, MultiLevelType multiLevelType) {
    ...
    if (multiLevelType == MultiLevelType.NONE) {
      if (wildcards < 1) {
        throw new IllegalArgumentException("wildcards < 1: " + wildcards);
      }
    } else {
      if (wildcards < 0) {
        throw new IllegalArgumentException("wildcards < 0: " + wildcards);
      }
    }
  ...
  public static Prefix valueOf(String prefix) {
    ...
    if (multiLevelType == MultiLevelType.NONE) {
      if (wildcards < 1) {
        throw new IllegalArgumentException("prefix does not end with any type of wildcard: " + prefix);
      }
    }
    Path base;
    if (prefixLen == 0) {
  ...
  private static int compare(Prefix p1, Prefix p2) {
    ...
    if (p1.multiLevelType != MultiLevelType.NONE) {
      effectiveWildcards1++;
    }
    int effectiveWildcards2 = p2.wildcards;
    if (p2.multiLevelType != MultiLevelType.NONE) {
      effectiveWildcards2++;
    }
  ...
  public boolean conflictsWith(Prefix other) {
    ...
    if (prefix1.multiLevelType != MultiLevelType.NONE) {
      effectiveWildcards1++;
    }
